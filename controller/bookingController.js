...
module.exports={
        ...
        try {
            ...
            if (!user) {
                ...
                if (bookData) {
                    ...
                    return res.send({ responseCode: 200, responseMessage: "your slot are in pending/delete",responseResult:[] })
                }
            }
        } catch (error) {
            console.log(error)
    ...
    cancelSlotByUser:async(req,res)=>{
        try {
            let query = { $and: [{_id:req.dataId}, { status: { $ne: "DELETE" } }, { userType: 'USER' }], };
            let user = await userModel.findOne(query);
            if (user) {
    ...
    cancelSlotBySAdmin:async(req,res)=>{
        try {
            let query = { $and: [{_id:req.dataId}, { status: { $ne: "DELETE" } }, { userType: "SUB-ADMIN" }], };
            let user = await subAdminModel.findOne(query);
            if (user) {
                let slotData=await bookingModel.findOne({$and:[{_id: req.body._id}, {status: "BOOKED" }]})
                if (slotData) {
                    let delSlot =await bookingModel.deleteOne({_id:slotData._id})
